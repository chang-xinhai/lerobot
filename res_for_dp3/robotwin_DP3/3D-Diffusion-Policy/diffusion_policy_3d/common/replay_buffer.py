    ...
    def copy_from_store(
        ...
        if store is None:
            ...
            for key in keys:
                ...
                cprint(f"Loading {key}: shape {arr.shape}, size {arr.nbytes / 1e9:.2f} GB", "green")
                
                # Load in chunks to reduce memory spike
                # Pre-allocate output array
                output = np.empty(arr.shape, dtype=arr.dtype)
                # Load chunk by chunk (1000 rows at a time)
                chunk_rows = 1000
                for i in range(0, arr.shape[0], chunk_rows):
                    end = min(i + chunk_rows, arr.shape[0])
                    output[i:end] = arr[i:end]
                data[key] = output
                
                cprint(f"  ✓ {key} loaded", "green")
