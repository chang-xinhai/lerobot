...
class LeRobotDatasetMetadata:
    ...
    #     """Keys to access visual modalities (regardless of their storage method)."""
    #     return [key for key, ft in self.features.items() if ft["dtype"] in ["depth"]]
...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self._preload_parallel_video_lock = False
        ...
        if self.preload:
            self.preload_dataset(parallelized=True)

    def preload_dataset(self, parallelized: bool = False, num_workers: int | None = None) -> None:
        """Preload all dataset items into memory.

        Args:
            parallelized: If True, preload items in parallel using a thread pool.
            num_workers: Number of worker threads used for parallel preloading. If None,
                defaults to os.cpu_count().
        """
        self._preloading = True
        total_items = len(self)
        logging.info(f"Preloading {total_items} items into memory with parallelized={parallelized}.")
        self._preloaded_items = [None] * total_items

        from tqdm.auto import tqdm

        if not parallelized or total_items == 0:
            self._preload_parallel_video_lock = False
            for idx in tqdm(range(total_items), desc=f"Preloading {self.repo_id}", unit="item"):
                item = self._compute_item(idx)
                self._preloaded_items[idx] = item
        else:
            self._preload_parallel_video_lock = True
            if num_workers is None:
                num_workers = os.cpu_count() or 16
            num_workers = max(16, num_workers)

            with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
                futures = {executor.submit(self._compute_item, idx): idx for idx in range(total_items)}
                for future in tqdm(
                    concurrent.futures.as_completed(futures),
                    total=total_items,
                    desc=f"Preloading {self.repo_id}",
                    unit="item",
                ):
                    idx = futures[future]
                    self._preloaded_items[idx] = future.result()
    ...
    def _query_videos(self, query_timestamps: dict[str, list[float]], ep_idx: int) -> dict[str, torch.Tensor]:
        ...
        for vid_key, query_ts in query_timestamps.items():
            ...
            video_path = self.root / self.meta.get_video_file_path(ep_idx, vid_key)
            if self._preload_parallel_video_lock:
