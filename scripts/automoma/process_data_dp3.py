...
def main():
    ...
    data_dir = os.path.join(load_dir, "data")
    ...
    first_valid_ep = None
    for ep in range(min(10, num)):  # Check first 10 episodes for a valid one
    ...
    if first_valid_ep is None:
        print("No valid episodes found! No dataset will be created.")
        raise SystemExit(1)
    ...
    try:
        ...
        if total_count == 0 or len(episode_ends_arrays) == 0:
            print("No valid episodes processed. Removing incomplete dataset.")
